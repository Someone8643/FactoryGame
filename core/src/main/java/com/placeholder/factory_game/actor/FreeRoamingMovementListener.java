...
public class FreeRoamingMovementListener extends InputListener {
    ...
    public boolean keyUp(InputEvent event, int keycode) {
        ...
        player.setStateAndVelocity(state != null ? state : PlayerState.STANDING_S, newVelocity);
    ...
    private PlayerState getPlayerStateBasedOnCurrentlyPressedKeys() {
        if (pressedKeyCodes.contains(Input.Keys.UP)) {
            if (pressedKeyCodes.contains(Input.Keys.RIGHT)) return PlayerState.WALKING_NE;
            if (pressedKeyCodes.contains(Input.Keys.LEFT))  return PlayerState.WALKING_NW;
            return PlayerState.WALKING_N;
        } else if (pressedKeyCodes.contains(Input.Keys.DOWN)) {
            if (pressedKeyCodes.contains(Input.Keys.RIGHT)) return PlayerState.WALKING_SE;
            if (pressedKeyCodes.contains(Input.Keys.LEFT))  return PlayerState.WALKING_SW;
